...
public class PlayerCameraController : MonoBehaviour
{
    ...
    public void RotateCamara(float inputVertical)
    {
        ...
        if (!Mathf.Approximately(inputVertical, 0)) //水平方向の入力が"ほぼ0"でないなら
        {
            ...
            m_rotationX -= inputVertical * cameraMoveSpeed * Time.deltaTime; //Unityは左手座標系なので、上下の回転角度（X軸中心）にはマイナスをかけなければならない
            m_rotationX = Mathf.Clamp(m_rotationX, -camRotateLimitX, camRotateLimitX); //縦方向(X軸中心)回転には角度制限をつけないと宙返りしてしまう
